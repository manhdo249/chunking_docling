...
from docling_chunking.search.package import Package
from docling_chunking.types.base import CollectionTypeEnum, StrictDateTime, UniqueList
...
class Meta(AliasModel, Generic[ClassificationT, DomainT], extra="forbid"):
    ...
    def version_has_schema(cls, v):
        ...
        docling_chunking = [item for item in v if item.name == "docling-core"]
        if not docling_chunking:
            raise ValueError(
                "the version should include at least a valid docling-core package"
            )
        elif len(docling_chunking) > 1:
            raise ValueError(
