...
Example:
    python docling_chunking/utils/generate_jsonschema.py doc.document.TableCell
...
def generate_json_schema(class_reference: str) -> Union[dict, None]:
    ...
    Args:
        class_reference: The reference to a class in 'docling_chunking.types'.

    Returns:
        A jsonable dict of the model's schema.
    """
    if not class_reference.startswith("docling_chunking.types."):
        class_reference = "docling_chunking.types." + class_reference
